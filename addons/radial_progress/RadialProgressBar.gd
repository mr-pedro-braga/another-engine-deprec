...
func _draw():
	draw_circle_arc(Vector2(0,0),radius,0,360,bg_color,0.5);
	angle = progress*360/max_value;
	draw_circle_arc(Vector2(0,0),radius-1,0,360,Color.black,0.5);
	draw_circle_arc(Vector2(0,0),radius,0,angle,bar_color,0.5);
	draw_circle_arc(Vector2(0,0),radius - thickness,0,360,bg_color,0.45);
	pass;
...
func draw_circle_arc(center, radius, angle_from, angle_to, color, squash):
	var nb_points = 32
	...
	for i in range(nb_points + 1):
		...
		points_arc.push_back(center + Vector2(cos(angle_point), sin(angle_point)*squash) * radius)
